#!/usr/bin/env python3
"""
Dual-Codebook VQ-VAE Training Script - 32 Embeddings CON VINCOLO

Questo script addestra un modello dual codebook con 32 embeddings totali
(16 active + 16 inactive) CON VINCOLO FORTE:
- Segnali inattivi → DEVONO usare codebook inactive
- Segnali attivi → DEVONO usare codebook active

Il vincolo è basato sull'attività REALE del segnale neurale, non sulla norma L2.

Uso:
    python scripts/train_dual_32_constrained.py
"""
...
# ============================================================================
# MODEL CONFIGURATION - DUAL CODEBOOK 32 EMBEDDINGS CON VINCOLO
# ============================================================================
...
MODEL_CONFIG = {
    ...
    # 🔥 DUAL CODEBOOK: 32 total = 16 active + 16 inactive
    'num_embeddings': 32,        # ⬅️ 32 embeddings totali
    'embedding_dim': 32,         # ⬅️ 32 dimensioni
...
def main():
    ...
    wandb.init(
        ...
        name="dual-32-constrained-single-neuron",
        config={
            "model_type": "DualCalciumVQVAE_Constrained_32",
        ...
        tags=["dual-codebook", "single-neuron", "constrained", "32-embeddings"]
    )
    
    print("🔥 DUAL-CODEBOOK VQ-VAE TRAINING (32 Embeddings CON VINCOLO)")
    print("="*70)
    print("✅ Model config:")
    ...
    try:
        ...
        # Create model
        print("\n🏗️  Creating dual codebook model (32 embeddings, CON VINCOLO)...")
        ...
        for epoch in range(TRAIN_CONFIG['max_epochs']):
            ...
            if epoch % TRAIN_CONFIG['evaluate_every'] == 0:
                ...
                if test_metrics['test/correlation_mean'] > best_test_corr:
                    ...
                    checkpoint_path = './results/dual_codebook/best_dual_model_32_constrained.pth'
                    torch.save({
                        'epoch': epoch,
