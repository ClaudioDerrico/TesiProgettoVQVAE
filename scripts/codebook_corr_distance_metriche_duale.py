#!/usr/bin/env python3
"""
Analisi Correlazione vs Distanza - DUAL Codebook Version
...
Uso:
    python codebook_corr_distance_metriche_duale.py
"""
...
# Directory output
OUTPUT_DIR = "correlazione_distanze_temporali_spettrali_dual"

# W&B Configuration
WANDB_PROJECT = "calcium-vqvae-correlation-distance-dual"
WANDB_RUN_NAME = f"corr-dist-dual-{datetime.now().strftime('%m%d-%H%M')}"
...
def plot_correlation_vs_distance_dual(pairs_data, base_code_idx, output_dir):
    ...
    fig.suptitle(f'Correlazione vs Distanza - Base Code {base_code_idx}', fontsize=16)
...
def main():
    print("="*70)
    print("📊 ANALISI CORRELAZIONE vs DISTANZA - DUAL Codebook Version")
    print("="*70)
    print(f"\n📋 Configurazione:")
    ...
    wandb.init(
        ...
        tags=["dual", "correlation", "distance", "codebook"]
    )
