#!/usr/bin/env python3
"""
Visualizzazione Scatter: TUTTE le Coppie di Codici DUAL CONSTRAINED

Versione DUAL CONSTRAINED che separa le coppie per tipo di codebook:
- Active-Active
- Active-Inactive
- Inactive-Active
- Inactive-Inactive
...
Uso:
    python scripts/plotta_dati_correlazione_distanza_duale_constrained.py
"""
...
# ✅ MODIFICA QUI: Directory dove sono salvati i dati per modelli constrained
# Se i dati constrained sono nella stessa directory, cambia solo il pattern di ricerca
PAIRS_DIR = "correlazione_distanze_temporali_spettrali_dual_constrained/coppie codice"

# Se la directory non esiste, prova a cercare nella directory standard
# con pattern che include "constrained" nel nome file
FALLBACK_DIR = "correlazione_distanze_temporali_spettrali_dual/coppie codice"

# Configurazione W&B
WANDB_PROJECT = "correlazione_vs_distanza_duale_constrained"
WANDB_RUN_NAME = "all-pairs-scatter-dual-constrained"
USE_WANDB = True
...
def load_all_pair_files(pairs_dir, fallback_dir=None):
    """
    Carica tutti i file coppie_codice_*.json
    Se la directory principale non esiste, prova il fallback.
    ...
    # Prova directory principale
    if not pairs_dir_path.exists() and fallback_path and fallback_path.exists():
        print(f"⚠️  Directory {pairs_dir} non trovata, uso fallback: {fallback_dir}")
        pairs_dir_path = fallback_path
    
    if not pairs_dir_path.exists():
        raise FileNotFoundError(f"Directory {pairs_dir} non trovata!")
    
    # Pattern: coppie_codice_*.json o coppiecodice*.json
    ...
    if not pair_files:
        raise FileNotFoundError(f"Nessun file coppie_codice_*.json trovato in {pairs_dir_path}/")
...
def plot_all_pairs_scatter_dual(data, use_wandb=False):
    """
    Crea 4 scatter plots con TUTTE le coppie
    ...
    # Converti None in NaN per numpy
    y = np.array([val if val is not None else np.nan for val in y])
    ...
    # Finalizzazione
    # ========================================================================
    plt.suptitle('Distance-Correlation Relationships: All Code Pairs DUAL CONSTRAINED', 
    ...
    if use_wandb:
        wandb.log({
            "all_pairs_scatter_dual_constrained": wandb.Image(fig)
        })
...
def print_detailed_statistics(data, correlations, correlations_by_type):
    ...
    # Converti None in NaN per le correlazioni
    temporal_corr = np.array([val if val is not None else np.nan for val in data['temporal_correlation']])
    spectral_corr = np.array([val if val is not None else np.nan for val in data['spectral_correlation']])
    
    print(f"\n🔵 TEMPORAL CORRELATION:")
    print(f"   Mean: {np.nanmean(temporal_corr):.4f}")
    print(f"   Std:  {np.nanstd(temporal_corr):.4f}")
    print(f"   Min:  {np.nanmin(temporal_corr):.4f}")
    print(f"   Max:  {np.nanmax(temporal_corr):.4f}")
    valid = ~np.isnan(temporal_corr)
    print(f"   Positive: {np.sum(temporal_corr[valid] > 0)} pairs ({100*np.sum(temporal_corr[valid] > 0)/np.sum(valid):.1f}%)")
    print(f"   Negative: {np.sum(temporal_corr[valid] < 0)} pairs ({100*np.sum(temporal_corr[valid] < 0)/np.sum(valid):.1f}%)")
    
    print(f"\n🟣 SPECTRAL CORRELATION:")
    print(f"   Mean: {np.nanmean(spectral_corr):.4f}")
    print(f"   Std:  {np.nanstd(spectral_corr):.4f}")
    print(f"   Min:  {np.nanmin(spectral_corr):.4f}")
    print(f"   Max:  {np.nanmax(spectral_corr):.4f}")
    valid = ~np.isnan(spectral_corr)
    print(f"   High (>0.9): {np.sum(spectral_corr[valid] > 0.9)} pairs ({100*np.sum(spectral_corr[valid] > 0.9)/np.sum(valid):.1f}%)")
...
def main():
    print("="*70)
    print("📊 SCATTER PLOT: TUTTE LE COPPIE DUAL CONSTRAINED")
    print("="*70)
    ...
    try:
        ...
        if USE_WANDB:
            ...
            wandb.init(
                ...
                name=WANDB_RUN_NAME,
                config={
                    "analysis_type": "all_pairs_scatter_dual_constrained",
                    "pairs_dir": PAIRS_DIR,
        ...
        all_pairs_data = load_all_pair_files(PAIRS_DIR, FALLBACK_DIR)
        ...
        if USE_WANDB:
            ...
            wandb.log({
                'descriptive/temporal_corr_mean': np.nanmean(temporal_corr),
                'descriptive/temporal_corr_std': np.nanstd(temporal_corr),
                'descriptive/spectral_corr_mean': np.nanmean(spectral_corr),
                'descriptive/spectral_corr_std': np.nanstd(spectral_corr),
            ...
            for pair_type in ['active-active', 'active-inactive', 'inactive-active', 'inactive-inactive']:
                ...
                if np.any(mask):
                    temporal_corr_type = np.array([val if val is not None else np.nan 
                                                   for val in data['temporal_correlation'][mask]])
                    spectral_corr_type = np.array([val if val is not None else np.nan 
                                                  for val in data['spectral_correlation'][mask]])
                    
                    wandb.log({
                        f'descriptive_by_type/{pair_type}/temporal_corr_mean': np.nanmean(temporal_corr_type),
                        f'descriptive_by_type/{pair_type}/spectral_corr_mean': np.nanmean(spectral_corr_type),
