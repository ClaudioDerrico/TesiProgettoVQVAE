#!/usr/bin/env python3
"""
Dual-Codebook VQ-VAE Training Script - CON VINCOLO (Configurabile)

Questo script addestra un modello dual codebook CON VINCOLO FORTE:
...
Uso:
    python scripts/train_dual_constrained.py
    
Per cambiare configurazione, modifica MODEL_CONFIG e TRAIN_CONFIG in questo file.
"""
...
# ============================================================================
# ⚙️ CONFIGURAZIONE - MODIFICA QUI PER CAMBIARE PARAMETRI
# ============================================================================
...
MODEL_CONFIG = {
    ...
    # 🔥 DUAL CODEBOOK: Modifica questi valori per cambiare numero embeddings
    # Esempi:
    #   32 embeddings:  num_embeddings=32,  embedding_dim=32
    #   64 embeddings:  num_embeddings=64,  embedding_dim=32
    #   128 embeddings: num_embeddings=128, embedding_dim=64
    #   256 embeddings: num_embeddings=256, embedding_dim=64
    #   512 embeddings: num_embeddings=512, embedding_dim=64
    #   1024 embeddings: num_embeddings=1024, embedding_dim=64
    'num_embeddings': 2048,        # ⬅️ CAMBIA QUI: 16, 32, 64, 128, 256, 512, 1024, etc.
    'embedding_dim': 128,         # ⬅️ CAMBIA QUI: tipicamente 32 per piccoli, 64 per grandi (>128)
...
def main():
    # Nome file checkpoint basato su num_embeddings
    num_emb = MODEL_CONFIG['num_embeddings']
    checkpoint_name = f'best_dual_model_{num_emb}_constrained.pth'
    
    # Initialize W&B
    wandb.init(
        project="calcium-vqvae-dual-codebook-constrained",
        name=f"dual-{num_emb}-constrained-single-neuron",
        config={
            "model_type": f"DualCalciumVQVAE_Constrained_{num_emb}",
        ...
        tags=["dual-codebook", "single-neuron", "constrained", f"{num_emb}-embeddings"]
    )
    
    print("🔥 DUAL-CODEBOOK VQ-VAE TRAINING (CON VINCOLO)")
    print("="*70)
    print("✅ Model config:")
    ...
    os.makedirs('./results/dual_codebook_constrained', exist_ok=True)
    ...
    try:
        ...
        # Create model
        print(f"\n🏗️  Creating dual codebook model ({num_emb} embeddings, CON VINCOLO)...")
        ...
        for epoch in range(TRAIN_CONFIG['max_epochs']):
            ...
            if epoch % TRAIN_CONFIG['evaluate_every'] == 0:
                ...
                if test_metrics['test/correlation_mean'] > best_test_corr:
                    ...
                    checkpoint_path = f'./results/dual_codebook/{checkpoint_name}'
                    torch.save({
                        'epoch': epoch,
