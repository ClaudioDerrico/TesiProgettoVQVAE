#!/usr/bin/env python3
"""
Analisi Temporale della Similarità del Codebook

Risponde alla domanda: "Perché due codici sono vicini/lontani nel clustering?"

Strategia:
1. Clustering gerarchico degli embeddings
...
Uso:
    python codebook_similarity_temporal_analysis.py
"""
...
# W&B Configuration
WANDB_PROJECT = "calcium-vqvae-codebook-similarity-temporal"
WANDB_RUN_NAME = f"similarity-temporal-{datetime.now().strftime('%m%d-%H%M')}"
...
def find_neighbor_and_distant_pairs(embeddings, linkage_matrix, num_neighbor_pairs, num_distant_pairs):
    ...
    Returns:
        neighbor_pairs: lista di (code_a, code_b, distance)
        distant_pairs: lista di (code_a, code_b, distance)
    """
    ...
    # TROVA COPPIE VICINE
    # ========================================================================
    # Le coppie più vicine hanno alta similarità (bassa distanza)
    
    neighbor_pairs = []
    
    # Itera sulla matrice triangolare superiore
    ...
    # Prendi le prime N coppie più vicine
    top_neighbor_pairs = neighbor_pairs[:num_neighbor_pairs]
    
    # ========================================================================
    # TROVA COPPIE DISTANTI
    # ========================================================================
    # Le coppie più distanti hanno bassa similarità (alta distanza)
    
    # Ordina per distanza decrescente (i più lontani per primi)
    distant_pairs_sorted = sorted(neighbor_pairs, key=lambda x: x[2], reverse=True)
    
    # Prendi le prime N coppie più distanti
...
def analyze_trace_characteristics(trace):
    ...
    characteristics = {
        ...
        'range': np.ptp(trace),  # peak-to-peak
    }
...
def compute_fft_spectrum(trace, sample_rate=1.0):
    ...
    Args:
        trace: (time,) traccia temporale
        sample_rate: frequenza di campionamento (default=1.0 Hz)
    
    Returns:
        freqs: array delle frequenze (Hz)
        power: spettro di potenza (magnitudine al quadrato)
        magnitude: magnitudine dello spettro
        phase: fase dello spettro
    """
    n = len(trace)
    
    # Rimuovi la componente DC (media) per concentrarsi sulle oscillazioni
...
def analyze_spectral_similarity(trace_a, trace_b):
    ...
    # Distanza L2 tra spettri normalizzati
    spectral_l2_dist = np.sqrt(np.sum((power_a_norm - power_b_norm) ** 2))
    
    # Frequenza dominante per ogni traccia
    if len(power_a) > 1:
        dominant_freq_a_idx = np.argmax(power_a[1:]) + 1  # Escludi DC (freq=0)
    ...
    # Differenza frequenza dominante
    freq_diff = abs(dominant_freq_a - dominant_freq_b)
    
    # Bandwidth (larghezza di banda): range di freq con potenza > 10% del max
...
def visualize_code_pair_comparison(code_a, code_b, trace_a, trace_b, 
                                   similarity, distance, pair_type):
    """
    Visualizza confronto dettagliato tra due codici (TEMPO + FREQUENZA)
    
    Args:
        code_a, code_b: indici dei codici
        trace_a, trace_b: tracce temporali (output_time,)
        similarity: cosine similarity
        distance: distance (1 - similarity)
        pair_type: "NEIGHBOR" o "DISTANT"
    """
    ...
    ax1.axhline(chars_a['mean'], color='cyan', linestyle='--', 
                       alpha=0.5, label=f"Mean: {chars_a['mean']:.3f}")
    ax1.fill_between(time_points, 
                            chars_a['mean'] - chars_a['std'],
                            chars_a['mean'] + chars_a['std'],
    ...
    ax2.axhline(chars_b['mean'], color='orange', linestyle='--', 
                       alpha=0.5, label=f"Mean: {chars_b['mean']:.3f}")
    ax2.fill_between(time_points, 
                            chars_b['mean'] - chars_b['std'],
                            chars_b['mean'] + chars_b['std'],
    ...
    # ========================================================================
    # ROW 2: ✨ NUOVO - Spettri di frequenza
    # ========================================================================
    ...
    ax3.axvline(spectral_analysis['dominant_freq_a'], color='cyan', 
    ...
    ax3.set_xlim([0, np.max(freqs) * 0.5])  # Show first half of spectrum
    
    # Spettro Code B
    ax4.plot(freqs, power_b, 'r-', linewidth=2, label=f'Code {code_b}')
    ax4.axvline(spectral_analysis['dominant_freq_b'], color='orange', 
                linestyle='--', linewidth=2,
    ...
    # Overlay temporale
    ax7.plot(time_points, trace_a, 'b-', linewidth=2, 
                    alpha=0.7, label=f'Code {code_a}')
    ax7.plot(time_points, trace_b, 'r-', linewidth=2, 
                    alpha=0.7, label=f'Code {code_b}')
    
    # Evidenzia differenze
    diff = trace_b - trace_a
    ax7.fill_between(time_points, trace_a, trace_b,
    ...
    # Calcola correlazione temporale
    ...
    # Feature comparison (temporal + spectral)
    ...
    # Normalizza per visualizzazione (tranne spectral_corr che è già 0-1)
    features_a_norm = []
    features_b_norm = []
    ...
    ax8.bar(x - width/2, features_a_norm, width, 
                   label=f'Code {code_a}', color='blue', alpha=0.7)
    ax8.bar(x + width/2, features_b_norm, width, 
                   label=f'Code {code_b}', color='red', alpha=0.7)
    ...
    # ========================================================================
    # TITLE e METRICHE GLOBALI
    # ========================================================================
    
    # Calcola similarità delle caratteristiche
    feature_diffs = [abs(features_a[i] - features_b[i]) for i in range(len(features_a))]
    
    # Identifica feature più simili e più diverse
    ...
    fig.suptitle(
        f'{pair_type} PAIR: Codes {code_a} ↔ {code_b}\n'
...
def analyze_why_similar_or_different(chars_a, chars_b):
    ...
    # Calcola differenze assolute e normalizzate
    abs_diffs = {}
    norm_diffs = {}
    ...
    # Ordina per differenza normalizzata
...
def main():
    print("="*70)
    print("🔬 ANALISI TEMPORALE SIMILARITÀ CODEBOOK")
    print("="*70)
    print(f"\n📋 Configurazione:")
    ...
    wandb.init(
        ...
        tags=["similarity", "temporal-analysis", "clustering", "codebook"]
    )
    ...
    try:
        ...
        print("\n🔬 Eseguendo clustering gerarchico...")
        similarity_matrix = cosine_similarity(embeddings)
        distance_matrix = 1 - similarity_matrix
        ...
        # 4. Trova coppie vicine e distanti
        print("\n🔍 Identificando coppie di codici...")
        ...
        # 5. Analizza coppie VICINE
        print("\n" + "="*70)
        print("🟢 ANALISI COPPIE VICINE")
        print("="*70)
        ...
        for pair_idx, (code_a, code_b, dist, sim) in enumerate(neighbor_pairs, 1):
            ...
            # Decodifica
            print(f"   Decodificando code {code_a}...")
            trace_a = decode_code(model, code_a, device, TEMPORAL_LENGTH)
            
            print(f"   Decodificando code {code_b}...")
            trace_b = decode_code(model, code_b, device, TEMPORAL_LENGTH)
            ...
            for feat in analysis['most_similar_features']:
                print(f"         {feat}: A={chars_a[feat]:.3f}, B={chars_b[feat]:.3f}, "
                      f"diff={analysis['abs_diffs'][feat]:.3f}")
            ...
            print(f"         Spectral L2 Distance: {spectral_analysis['spectral_l2_distance']:.4f}")
            
            # Visualizza
            fig = visualize_code_pair_comparison(
                code_a, code_b, trace_a, trace_b, sim, dist, "NEIGHBOR"
            )
            
            wandb.log({f"neighbor_pairs/pair_{pair_idx}_codes_{code_a}_{code_b}": wandb.Image(fig)})
            plt.close(fig)
        
        # 6. Analizza coppie DISTANTI
        print("\n" + "="*70)
        print("🔴 ANALISI COPPIE DISTANTI")
        print("="*70)
        ...
        for pair_idx, (code_a, code_b, dist, sim) in enumerate(distant_pairs, 1):
            ...
            for feat in analysis['most_different_features']:
                print(f"         {feat}: A={chars_a[feat]:.3f}, B={chars_b[feat]:.3f}, "
                      f"diff={analysis['abs_diffs'][feat]:.3f}")
            ...
            # Visualizza
            fig = visualize_code_pair_comparison(
                code_a, code_b, trace_a, trace_b, sim, dist, "DISTANT"
            )
            
            wandb.log({f"distant_pairs/pair_{pair_idx}_codes_{code_a}_{code_b}": wandb.Image(fig)})
            plt.close(fig)
        ...
        print(f"\n✅ Analizzate {NUM_NEIGHBOR_PAIRS} coppie vicine")
        print(f"✅ Analizzate {NUM_DISTANT_PAIRS} coppie distanti")
