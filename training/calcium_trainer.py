...
class CalciumTrainer:
    ...
    SIMPLIFIED VERSION: Uses only train/test split (no validation set).
    Test set is used for both early stopping and final evaluation.
    """
    ...
    def __init__(self, model, train_loader, test_loader, device='cuda', 
        ...
        Args:
            ...
            test_loader: test dataloader (used for both validation and final test)
        ...
        self.test_loader = test_loader  # Used as both validation and test
        self.device = device
        ...
        self.best_test_loss = float('inf')  # Changed from val to test
        self.best_epoch = 0
        ...
        print(f"Using simplified train/test split (test set used for early stopping)")
    
    def train(self, num_epochs=100, learning_rate=1e-4, behavior_weight=0.01,
              patience=20, gradient_clip=1.0, save_best=True, eval_interval=5):
        """
        Main training loop with simplified train/test split.
        ...
        print(f"Test set used for early stopping and final evaluation")
        ...
        for epoch in range(num_epochs):
            ...
            # Test phase (used as validation for early stopping)
            ...
            # Early stopping check (based on test set)
        ...
        # Final evaluation (same test set, but loaded from best model)
    ...
    def _test_epoch(self, recon_loss_fn, behavior_loss_fn, behavior_weight):
        """Test for one epoch (used as validation during training)."""
        self.model.eval()
        ...
        with torch.no_grad():
            for neural_data, behavior_data in self.test_loader:
                neural_data = neural_data.to(self.device)
                behavior_data = behavior_data.to(self.device)
    ...
    def _log_metrics(self, epoch, train_metrics, test_metrics):
        ...
        for key, value in test_metrics.items():
            self.writer.add_scalar(f'Test/{key}', value, epoch)  # Changed from Val to Test
        ...
        for key, value in test_metrics.items():
            self.metrics[f'test_{key}'].append(value)  # Changed from val to test
    ...
    def _print_progress(self, epoch, train_metrics, test_metrics, epoch_time, total_time):
        ...
        print(f"  Test  | Recon: {test_metrics['recon_loss']:.4f}, "
              ...
              f"Behavior: {test_metrics['behavior_loss']:.4f}, "
              f"Total: {test_metrics['total_loss']:.4f}")
        
        if 'behavior_r2' in test_metrics:
            print(f"        | Behavior R²: {test_metrics['behavior_r2']:.3f}")
    ...
    def _save_checkpoint(self, epoch, is_best=False):
        ...
        checkpoint = {
            ...
            'best_test_loss': self.best_test_loss,  # Changed from val to test
    ...
    def _evaluate_on_loader(self, loader, dataset_name):
        ...
        with torch.no_grad():
            for neural_data, behavior_data in loader: #loader=test_loader
                neural_data = neural_data.to(self.device)
                behavior_data = behavior_data.to(self.device)
    ...
    def _save_results(self, results):
        ...
        try:
            ...
            basic_results = {
                ...
                'best_test_loss': float(results.get('best_test_loss', 0.0)),
        ...
        if 'final_test_metrics' in results and 'reconstruction_examples' in results['final_test_metrics']:
    ...
    def _plot_training_curves(self, metrics):
        ...
        axes[0, 0].plot(epochs, metrics['test_recon_loss'], label='Test', alpha=0.7)  # Changed from val
        ...
        axes[0, 1].plot(epochs, metrics['test_vq_loss'], label='Test', alpha=0.7)  # Changed from val
        ...
        axes[1, 0].plot(epochs, metrics['test_perplexity'], label='Test', alpha=0.7)  # Changed from val
        ...
        if 'train_behavior_loss' in metrics and any(x > 0 for x in metrics['train_behavior_loss']):
            ...
            axes[1, 1].plot(epochs, metrics['test_behavior_loss'], label='Test', alpha=0.7)  # Changed from val
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        self.model.load_state_dict(checkpoint['model_state_dict'])
        self.best_test_loss = checkpoint.get('best_test_loss', float('inf'))  # Changed from val
...
def create_trainer_from_config(model, dataloaders, config):
    """
    Create trainer from configuration dictionary (simplified for train/test).
    
    Args:
        model: CalciumVQVAE model
        dataloaders: tuple of (train_loader, test_loader)
...
def train_calcium_vqvae(model, dataloaders, training_config=None):
    """
    High-level function to train a calcium VQ-VAE model (simplified train/test).
    ...
    # Create trainer (simplified for train/test)
    trainer = CalciumTrainer(
        model=model,
        train_loader=dataloaders[0],
        test_loader=dataloaders[1],  # Only train and test
...
if __name__ == "__main__":
    # Test trainer setup
    print("Testing CalciumTrainer (simplified train/test)...")
    ...
    test_loader = DataLoader(dataset[120:], batch_size=32, shuffle=False)   
    
    # Create model
    model = CalciumVQVAE(
        num_neurons=50,
        num_hiddens=128, #CAMBIATO
    ...
    print(f"Final test loss: {results['best_test_loss']:.6f}")
