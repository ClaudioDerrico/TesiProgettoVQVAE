"""
Simplified utility functions for working with Allen Brain Observatory data.
Uses fixed session 501940850 for consistency.
"""
...
def load_session_data(session_id=501940850, cache_dir=None):
    """
    Load data from the fixed Allen Brain Observatory session.
    
    Args:
        session_id: Allen Brain session ID (default: 501940850)
    ...
    print(f"Loading fixed session {session_id}...")
...
if __name__ == "__main__":
    # Test the utility functions with fixed session
    print("Testing Allen Brain utilities with fixed session...")
    
    # Test loading data from fixed session
    try:
